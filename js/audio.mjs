/** CC0-referenser: Kenney RPG Audio + artisticdude RPG Sound Pack (se sfx/LICENSE.txt).
 *  Spelet syntar korta SFX i Web Audio så vi slipper tunga assets. */

export const SFX_KEYS = [
  "slash", "swing", "hit", "loot", "click", "portal", "bag", "die", "magic", "level", "kill",
];
...
export function playSfx(name, { volume = 0.55 } = {}) {
  ...
  if (!SFX_KEYS.includes(name) && !PATCH[name]) return false;
  const a = ctx();
  if (!a) return false;
  try {
    const fn = PATCH[name];
